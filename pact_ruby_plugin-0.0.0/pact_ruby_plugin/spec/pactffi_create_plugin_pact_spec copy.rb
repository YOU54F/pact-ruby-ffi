...
RSpec.describe 'test pact plugin protobuf consumer' do
  describe 'InitPluginRequest' do
    ...
    it 'executes the pact test without errors' do
      ...
      p 'sending pre foo'
      p 'sending foo'
      PactRubyFfi.pactffi_interaction_contents(message_pact, 0, 'application/protobuf', JSON.dump(contents))
      mock_server_port = PactRubyFfi.pactffi_create_mock_server_for_transport(pact, '127.0', 0, 'grpc', nil)
      p mock_server_port
